     ...
     async updateById(id, object){
        try {            
            ...
            if(i != -1) {
                ...
                return {error: 'id '+id+' no pudo ser actualizado ya que no se encontró'};
            }
...
/*(async () => {
  ...
  async function createInitialProducts() { 
    await contenedor.save(new Producto('Black Album',1500, 'http://album.image.com'));
    await contenedor.save(new Producto('Zeppelin IV',4400, 'http://album.image.com'));
    await contenedor.save(new Producto('Physical Graffiti',5400, 'http://album.image.com'));
  ...
  async function insertProduct(object){
      return await contenedor.save(new Producto(object.title, parseInt(object.price),object.thumbnail));
  }
